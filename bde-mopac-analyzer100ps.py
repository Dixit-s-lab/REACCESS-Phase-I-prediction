...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="REACCESS Core Analytics Engine - Dynamic Stride & Full Graphics Suite."
    )
...
def main():
    ...
    if abs_trajectory_path:
        ...
        print(f"--> [Occupancy Core] To achieve >= 100 ps, atoms must be active for a minimum of {min_required_frames} frames.")
        
        df_carbons_geo = df_geo[df_geo['Element'] == 'C']
        if not df_carbons_geo.empty:
            passed_frames_c = df_carbons_geo.groupby('RDKit_Idx')['Passes_Both'].sum()
            active_carbon_ids = list(passed_frames_c[passed_frames_c >= min_required_frames].index)
        else:
            active_carbon_ids = []

        df_hetero_geo = df_geo[df_geo['Element'].isin(['N', 'S'])]
        if not df_hetero_geo.empty:
            passed_frames_h = df_hetero_geo.groupby('RDKit_Idx')['Passes_Distance'].sum()
            active_hetero_ids = list(passed_frames_h[passed_frames_h >= min_required_frames].index)
        else:
            active_hetero_ids = []
    else:
    ...
    # FIXED SECTION: Generating all 1D, 2D and 3D charts inside the active workspace folder
    if not bde_df.empty:
    ...
    # FIXED SECTION: Generating pristine 2D ACS highlighted layout map directly in workspace
