...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Master Automated MOPAC PM7 BDE Engine via REACCESS Framework Platforms."
    )
...
def main():
    ...
    if abs_trajectory_path:
        print(f"--> Trajectory file detected. Parsing tracking coordinates...")
    ...
    # Explicitly supply FE vdw radius to help guess_bonds as a backup
    if not hasattr(u, 'bonds') or len(u.bonds) == 0:
        try:
            u.atoms.guess_bonds(vdwradii={'FE': 1.8, 'Fe': 1.8})
        except:
            pass
    ...
    for f_idx, ts in enumerate(frames_loop):
        ...
        for atom in ligand_atoms:
            ...
            # Spatial distance lookup handles explicit hydrogen targeting flawlessly
    ...
    if not geometric_records:
        raise ValueError("[ERROR] Matrix parsing failure: Zero geometry arrays mapped down to memory data frames.")

    df_geo = pd.DataFrame(geometric_records)

    # Filter out reactive centers that cross the 5% occupancy constraint thresholds
    df_carbons_geo = df_geo[df_geo['Element'] == 'C']
    if not df_carbons_geo.empty:
        passed_frames_pct_c = df_carbons_geo.groupby('RDKit_Idx')['Passes_Both'].sum() / total_trajectory_frames * 100.0
        active_carbon_ids = list(passed_frames_pct_c[passed_frames_pct_c >= 5.0].index)
    else:
        active_carbon_ids = []

    df_hetero_geo = df_geo[df_geo['Element'].isin(['N', 'S'])]
    if not df_hetero_geo.empty:
        passed_frames_pct_h = df_hetero_geo.groupby('RDKit_Idx')['Passes_Distance'].sum() / total_trajectory_frames * 100.0
        active_hetero_ids = list(passed_frames_pct_h[passed_frames_pct_h >= 5.0].index)
    else:
        active_hetero_ids = []
    ...
    print(f"--> [Status] Isolated {total_unique_carbons} unique carbon centers passing >= 5% frame occupancy for quantum analysis.")
    print(f"--> [Status] Isolated {len(active_hetero_ids)} unique heteroatom centers (N/S) passing >= 5% distance occupancy.")

    # FIXED: Direct write out to destination output directory folder bypassing legacy move errors
